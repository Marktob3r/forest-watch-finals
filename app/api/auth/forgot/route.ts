...
import { findUserByEmail, generatePasswordResetCodeForUser } from '@/lib/auth';
...
async function sendWithSendGrid(toEmail: string, code: string) {
  ...
  const payload = {
    personalizations: [
      {
        to: [{ email: toEmail }],
        subject: 'Password reset code for Forest Watch',
      },
    ],
    from: { email: FROM_EMAIL, name: 'Forest Watch' },
    content: [
      { type: 'text/plain', value: `Your Forest Watch password reset code is ${code}. It expires in 15 minutes.` },
      { type: 'text/html', value: `<p>Your Forest Watch password reset code is <strong>${code}</strong>. It expires in 15 minutes.</p>` },
    ],
  };
...
export async function POST(req: Request) {
  try {
    ...
    const result = await generatePasswordResetCodeForUser(email);
